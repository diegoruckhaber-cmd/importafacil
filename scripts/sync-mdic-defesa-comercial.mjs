...
const UNIT_PATTERNS = [
  ...
  ["USD_PER_SQUARE_METER", /(?:US\$|USD)\s*([\d.,]+)\s*\/\s*m(?:²|2|etros?\s*quadrados?)/i],
  ["AD_VALOREM", /([\d.,]+)\s*%/],
];

const SUFFIX_PATTERNS = [
  ["USD_PER_THOUSAND_UNITS", /([\d.,]+)\s*(?:US\$|USD)\s*\/\s*(?:mil\s*unidades|milheiro|milheiros)/i],
  ["USD_PER_KG", /([\d.,]+)\s*(?:US\$|USD)\s*\/\s*kg/i],
  ["USD_PER_TON", /([\d.,]+)\s*(?:US\$|USD)\s*\/\s*(?:t|ton|tonelada|toneladas)/i],
  ["USD_PER_PAIR", /([\d.,]+)\s*(?:US\$|USD)\s*\/\s*par/i],
  ["USD_PER_UNIT", /([\d.,]+)\s*(?:US\$|USD)\s*\/\s*(?:unidade|unidades|un)/i],
  ["USD_PER_SQUARE_METER", /([\d.,]+)\s*(?:US\$|USD)\s*\/\s*m(?:²|2|etros?\s*quadrados?)/i],
];

const HEADER_PATTERNS = [
  ["USD_PER_THOUSAND_UNITS", /(?:US\$|USD)\s*\/\s*(?:mil\s*unidades|milheiro|milheiros)/i],
  ["USD_PER_KG", /(?:US\$|USD)\s*\/\s*kg/i],
  ["USD_PER_TON", /(?:US\$|USD)\s*\/\s*(?:t|ton|tonelada|toneladas)/i],
  ["USD_PER_PAIR", /(?:US\$|USD)\s*\/\s*par/i],
  ["USD_PER_UNIT", /(?:US\$|USD)\s*\/\s*(?:unidade|unidades|un)/i],
  ["USD_PER_SQUARE_METER", /(?:US\$|USD)\s*\/\s*m(?:²|2|etros?\s*quadrados?)/i],
  ["AD_VALOREM", /ad\s*valorem|%/i],
];
...
function stripOriginDecoration(value) {
  return clean(value)
    .replace(/^[\-–—•]+\s*/, "")
    .replace(/[\*\.,:;]+$/g, "")
    .replace(/\s*\([^)]*\)\s*$/g, "")
...
function origin(value) {
  const base = stripOriginDecoration(value);
  const x = base.toLowerCase();
  const map = {
    ...
    "do reino da tailândia": "Tailândia",
    "taipé chinês": "Taipé Chinês",
...
function detectRate(cells, headerUnit) {
  const line = cells.join(" | ");
  for (const [unit, pattern] of [...UNIT_PATTERNS, ...SUFFIX_PATTERNS]) {
    const match = line.match(pattern);
    if (match) return { unit, rate: number(match[1]) };
  }
  if (headerUnit && /\bzero\b|\bzerado\b/i.test(line)) return { unit: headerUnit, rate: 0 };
  if (headerUnit) {
    for (let i = cells.length - 1; i >= 0; i--) {
      const cell = cells[i];
      if (/prazo\s+(?:da|de)\s+vig[eê]ncia|vig[eê]ncia|^fonte/i.test(cell)) continue;
      const match = cell.match(/(?<![\d.,])([\d]{1,3}(?:\.\d{3})*(?:,\d+)?|[\d]+(?:,\d+)?)\s*%?\s*$/);
      if (match) return { unit: headerUnit, rate: number(match[1]) };
    }
  }
  return null;
}

function extractExporter(cells, line, options) {
  if (cells.length >= 3) {
    const candidateOrigin = origin(cells[0]);
    if (options[candidateOrigin.toLowerCase()]) return { originFromRow: candidateOrigin, exporter: cells[cells.length - 2] };
    return { exporter: cells[0] };
  }
  if (cells.length === 2) return { exporter: cells[0] };
  const prose = line.split(/\s*(?:=|–|—)\s*(?=(?:US\$|USD|zero\b|zerado\b|[\d.,]+\s*%))/i)[0];
  return { exporter: prose };
}

function addOption(options, currentOrigin, exporter, detected, line) {
  if (!currentOrigin || !detected || !Number.isFinite(detected.rate)) return;
  let cleanExporter = clean(exporter).replace(/^[-–—•:\s]+|[-–—•:\s]+$/g, "").replace(/\s*\|\s*$/g, "").trim();
  if (!cleanExporter || /^(Direito|Origem|Produtor\/Exportador)$/i.test(cleanExporter)) cleanExporter = "Todos os produtores/exportadores";
...
export function parsePage(url, html) {
  ...
  const headerUnit = detectHeaderUnit(sectionText);
  ...
  for (const line of sectionLines) {
    ...
    const headingOrigin = origins.find((candidate) => origin(candidate).toLowerCase() === normalizedLineOrigin.toLowerCase());
    if (headingOrigin) currentOrigin = headingOrigin;

    for (const candidate of origins) {
      const normalizedCandidate = origin(candidate);
      if (normalizedLineOrigin.toLowerCase().startsWith(`${normalizedCandidate.toLowerCase()}:`)) currentOrigin = candidate;
      if (line.toLowerCase().includes(`${normalizedCandidate.toLowerCase()}:`)) currentOrigin = candidate;
    }

    const detected = detectRate(cells.length ? cells : [line], headerUnit);
    if (!detected) continue;
    const extracted = extractExporter(cells, line, options);
    if (extracted.originFromRow) currentOrigin = extracted.originFromRow;
    addOption(options, currentOrigin, extracted.exporter, detected, line);
  }

  for (const key of Object.keys(options)) {
    const seen = new Set();
    options[key] = options[key].filter((x) => {
      if (/prazo\s+(?:da|de)\s+vigência/i.test(x.exporter)) return false;
      const sig = `${x.exporter}|${x.rate}|${x.unit}`;
  ...
  return {
    ncm: ncm.patterns[0],
    ncmPatterns: ncm.patterns,
    ncmExclusions: ncm.exclusions,
    product: title,
    origins,
    measure: "antidumping",
    measureTypeText: clean(type),
...
export async function syncMdICDefenseCatalog() {
  ...
  const missing = measures.flatMap((item) => Object.entries(item.exportersByOrigin).filter(([, exporterOptions]) => exporterOptions.length === 0).map(([itemOrigin]) => ({ ncm: item.ncm, product: item.product, origin: itemOrigin, sourceUrl: item.sourceUrl })));
  const summary = { indexPages: urls.length, antidumpingMeasures: measures.length, missingExporterOptionCount: missing.length, missingExporterOptions: missing, failures: failures.slice(0, 20) };
  console.log(JSON.stringify(summary, null, 2));
  return summary;
}

const invokedAsScript = process.argv[1] && pathToFileURL(path.resolve(process.argv[1])).href === import.meta.url;
if (invokedAsScript) await syncMdICDefenseCatalog();
