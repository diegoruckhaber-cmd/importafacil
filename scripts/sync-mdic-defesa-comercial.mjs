...
function origin(value) {
  ...
  const map = {
    "coréia": "Coreia do Sul", "coréia do sul": "Coreia do Sul", "coreia": "Coreia do Sul", "coreia do sul": "Coreia do Sul",
...
function addOption(options, currentOrigin, exporter, detected, line) {
  ...
  let cleanExporter = clean(exporter).replace(/^[-–—•:\s]+|[-–—•:\s]+$/g, "").trim();
  cleanExporter = cleanExporter.replace(/\s*\|\s*$/g, "").trim();
...
function parsePage(url, html) {
  ...
  for (const line of sectionLines) {
    ...
    for (const candidate of origins) {
      if (new RegExp(`(?:^|[|,;])\\s*${candidate.replace(/[.*+?^${}()|[\\]\\]/g, "\\$&")}\\s*(?:[|,;]|$)`, "i").test(line) || origin(line) === candidate) currentOrigin = candidate;
    }
    ...
    if (cells.length >= 3) {
      ...
      if (options[candidateOrigin.toLowerCase()]) {
        currentOrigin = candidateOrigin;
        exporter = cells[cells.length - 2];
      } else exporter = cells[0];
    } else if (cells.length === 2) {
      exporter = cells[0];
    } else {
      exporter = line.replace(/(?:US\$|USD)\s*[\d.,]+\s*\/\s*\S+/i, "").replace(/[\d.,]+\s*%/, "").replace(/[\d]{1,3}(?:\.\d{3})*(?:,\d+)?\s*$/, "").trim();
    }
  ...
  for (const key of Object.keys(options)) {
    ...
    options[key] = options[key].filter((x) => {
      if (/prazo\s+(?:da|de)\s+vig[eê]ncia/i.test(x.exporter)) return false;
  ...
  const titleIndex = lines.findIndex((line) => /^#?\s*[^#]+$/.test(line) && /Compartilhe|Info|Informações/.test(lines[lines.indexOf(line) + 1] ?? ""));
...
console.log(JSON.stringify({ indexPages: urls.length, antidumpingMeasures: measures.length, missingExporterOptionCount: measures.filter((item) => Object.values(item.exportersByOrigin).some((options) => options.length === 0)).length, failures: failures.slice(0, 20) }, null, 2));
