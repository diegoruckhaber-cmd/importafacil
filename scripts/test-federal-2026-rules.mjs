...
const normal = resolveCofinsImport2026({ date: "2026-03-31", reducedBenefit: true });
assert.equal(normal.effectiveRate, 9.65);

const reduced = resolveCofinsImport2026({ date: "2026-04-01", reducedBenefit: true });
assert.equal(reduced.effectiveRate, 0.965);
assert.equal(reduced.displayRate, 0.97);

const reducedPlus = resolveCofinsImport2026({ date: "2026-04-01", reducedBenefit: true, additional060: true });
...
console.log("Federal 2026 rule resolver: PASS");
