...
const close = (actual, expected, tolerance = 0.01) => assert(Math.abs(actual - expected) <= tolerance, `expected ${actual} to be within ${tolerance} of ${expected}`);
...
const input = { date: "2026-09-10", destinationUf: "SP", exchange: 5.5, freight: 100, insurance: 10, transportMode: "air", declarationType: "di", items: [{ itemId: "ITEM-001", name: "Verniz", ncm: "32081020", origin: "México", quantity: 10, weightKg: 100, fobUnit: 20, icms: 4, ttd: "none", destination: "commercial_resale" }] };
...
assert(result.jurisdiction.homologatedUfs.includes("SP"));
...
assert.equal(row.icmsNormalRate, 18);
...
assert(activation.activeUfs.includes("SP"));
console.log("Stage 19 SP general ICMS: OK — 18% canonical rate remains stable as state coverage expands");
