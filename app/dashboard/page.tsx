...
export default function Dashboard(){
 ...
 useEffect(()=>{(async()=>{const {data:{user}}=await supabase.auth.getUser();if(!user){window.location.href="/auth";return;}setEmail(user.email||"");const {data:profile}=await supabase.from("profiles").select("plan").eq("id",user.id).maybeSingle();if(profile?.plan)setPlan(profile.plan);const {data}=await supabase.from("simulations").select("id,name,input,result,created_at").order("created_at",{ascending:false}).limit(50);setItems((data||[]) as Simulation[]);setLoading(false)})()},[]);
 async function logout(){await supabase.auth.signOut();window.location.href="/";}
 const isFree=plan==="FREE";const shown=isFree?items.slice(0,3):items;
 ...
 <section style={{maxWidth:1100,margin:"auto",padding:"50px 24px"}}><div style={{display:"flex",justifyContent:"space-between",gap:20,alignItems:"end",marginBottom:30}}><div><small style={{letterSpacing:1,color:"#777"}}>MINHA CONTA</small><h1 style={{fontSize:40,margin:"8px 0"}}>Suas simulações</h1><p style={{color:"#666"}}>{isFree?"Seu plano gratuito inclui até 3 simulações salvas.":"Histórico completo das suas simulações."}</p></div><a href="/#simulador" style={{padding:"12px 16px",background:"#111",color:"white",borderRadius:10,textDecoration:"none",fontWeight:700}}>Nova simulação</a></div>
 {loading?<p>Carregando...</p>:shown.length===0?<div style={{background:"white",border:"1px solid #e5e5df",borderRadius:18,padding:40}}><h2>Seu histórico está vazio.</h2><p style={{color:"#666"}}>Faça uma simulação e salve o resultado para começar.</p></div>:<div style={{display:"grid",gap:12}}>{shown.map(x=><article key={x.id} style={{background:"white",border:"1px solid #e5e5df",borderRadius:16,padding:20,display:"flex",justifyContent:"space-between",gap:20}}><div><b>{x.name||"Simulação de importação"}</b><small style={{display:"block",color:"#888",marginTop:5}}>{new Date(x.created_at).toLocaleString("pt-BR")}</small></div><div style={{textAlign:"right"}}><small style={{color:"#888"}}>Custo total</small><div style={{fontWeight:800,fontSize:20}}>{br(x.result.total)}</div></div></article>)}</div>}
 {isFree&&items.length>=3&&<div style={{marginTop:18,padding:18,borderRadius:14,background:"#fff7df",border:"1px solid #ead9a3"}}>Você atingiu o limite de 3 simulações do plano FREE. <a href="/#pro" style={{fontWeight:700,color:"#111"}}>Conheça o PRO</a> para liberar histórico completo.</div>}
