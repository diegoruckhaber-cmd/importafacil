...
async function persistCalculatedRecord(auth: Authenticated, name: string, input: unknown, result: unknown): Promise<NextResponse> {
  ...
  if (saved.limited) return NextResponse.json({ error: "Você atingiu o limite de 3 simulações do plano FREE. Exclua uma simulação do histórico ou conheça o PRO." }, { status: 403 });
