...
export function calculateUnifiedImportSimulation(input: UnifiedImportSimulationInput) {
  ...
  const normalizedItems = input.items.map((item, index) => {
    ...
    return {
      ...
      fobUnit: Number(item.fobUnit),
      icms: Number(item.icms),
      ttd: item.ttd ?? "none",
      destination: item.destination ?? "commercial_resale",
    };
  });
  ...
  const additionalCharges = resolveSCImportAdditionalCharges({
    freightBrl,
    transportMode: input.transportMode ?? "not_informed",
    declarationType: input.declarationType ?? "di",
    additions: Math.max(1, Math.floor(Number(input.additions ?? normalizedItems.length))),
  });
  ...
  const itemResolutions = normalizedItems.map((item, index) => {
    ...
    const defense = resolveDefenseCommercial({
      ...
      weightKg: item.weightKg > 0 ? item.weightKg : undefined,
      quantity: item.quantity,
      exporter: item.exporter || undefined,
      exchangeRate: exchange,
      customsValueBrl: merchandiseValueBrl + allocatedFreightBrl + allocatedInsuranceBrl,
    });
    ...
    if (usesTtd || specialRegimeIds.length > 0) {
      scDecision = decideSCItem({
        id: item.itemId,
        ttd: usesTtd ? Number(item.ttd) as 77 | 409 | 410 : undefined,
        destination: item.destination,
        validConcession: item.validConcession === true,
        importEntryInSC: item.importEntryInSC !== false,
        decree2128Prohibited: item.decree2128Prohibited === true,
        sameNcmPositionAfterFractionation: item.sameNcmPositionAfterFractionation !== false,
        specialRegimeIds,
    ...
    if (usesTtd) {
      const benefit = resolveSCBenefit({
        ttd: Number(item.ttd) as 77 | 409 | 410,
        destination: item.destination,
        ncm: item.ncm,
        normalOutputICMS: 0,
        taxableOutput: true,
        industrializationInSC: item.destination === "industrialization" && item.industrializationInSC === true,
        preservesOriginalCharacteristics: item.sameNcmPositionAfterFractionation !== false,
        sameNcmPosition: item.sameNcmPositionAfterFractionation !== false,
    ...
    return {
      ...
      merchandiseValueBrl,
      allocatedFreightBrl,
      allocatedInsuranceBrl,
      federal: { ...federal, origin: item.origin, defenseCommercial: defense },
      sc: {
        ttd: item.ttd,
        specialRegimeIds,
        decision: scDecision?.decision ?? "normal",
        decisionReasons: [
          ...(scDecision?.reasons ?? ["Nenhum TTD/regime especial informado; tributação normal preservada."]),
          ...federal.warnings,
          ...additionalCharges.warnings,
          ...((defense.status === "requires_input" || defense.status === "identified") ? defense.warnings : []),
        ],
        ruleIds: scDecision?.ruleIds ?? [],
        benefitDecision: benefitsByItem[item.itemId]?.decision ?? "normal",
        benefitReasons: benefitsByItem[item.itemId]?.reasons ?? [],
        blockingIssues: [...new Set([...(scDecision?.blockingIssues ?? []), ...(benefitsByItem[item.itemId]?.blockingIssues ?? [])])],
      },
      defenseCommercialBrl: defense.amountBrl ?? 0,
    };
  });

  const expenses: ItemImportExpense[] = [];
  const addExpense = (expense: UnifiedImportExpenseInput) => {
  ...
  const engineItems = normalizedItems.map((item, index) => {
    const federal = itemResolutions[index].federal;
    return {
      itemId: item.itemId,
      customsValue: merchandiseValues[index],
      quantity: item.quantity,
  ...
  const calculationItems = calculation.items.map((calculatedItem) => {
    const resolution = resolutionById.get(calculatedItem.itemId)!;
    const defenseCommercialBrl = resolution.defenseCommercialBrl;
    const landedCostIncludingDefense = calculatedItem.landedCostAfterBenefit + defenseCommercialBrl;
    return {
      ...calculatedItem,
      defenseCommercial: resolution.federal.defenseCommercial,
      defenseCommercialBrl,
      landedCostIncludingDefense,
      landedCostPerUnitIncludingDefense: resolution.quantity > 0 ? landedCostIncludingDefense / resolution.quantity : landedCostIncludingDefense,
    };
  });
  ...
  if (items.length === 1) {
    return {
      ...response,
      federal: items[0].federal,
      sc: {
        ...items[0].sc,
        icmsNormalRate: items[0].calculation.icmsNormalRate,
        icmsImportEffectiveRate: items[0].calculation.icmsImportEffectiveRate,
        normalImportICMS: items[0].calculation.normalImportICMS,
        effectiveImportICMS: items[0].calculation.benefitImportICMS,
        importICMSSavings: items[0].calculation.importICMSSavings,
