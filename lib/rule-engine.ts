...
export type RuleCondition = { field: string; operator: 'eq' | 'neq' | 'in' | 'not_in' | 'gte' | 'lte' | 'gt' | 'lt' | 'exists'; value?: string | number | boolean | Array<string | number> };
export type RuleAction = { type: 'set_rate' | 'set_base_component' | 'apply_factor' | 'flag' | 'block'; target: string; value?: number | string | boolean };
export type TaxRule = { id: string; jurisdiction: string; tax: string; effectiveFrom: string; effectiveTo?: string; priority: number; conditions: RuleCondition[]; actions: RuleAction[]; legalBasis: string; source: string; confidence: RuleConfidence };
export type RuleEvaluation = { ruleId: string; matched: boolean; applicable: boolean; missingFields: string[]; reasons: string[]; confidence: RuleConfidence };
...
function conditionMatches(condition: RuleCondition, context: RuleContext): { matched: boolean; missing: boolean } {
  ...
  switch (condition.operator) {
    ...
    case 'gt': return { matched: Number(actual) > Number(condition.value), missing: false };
    case 'lt': return { matched: Number(actual) < Number(condition.value), missing: false };
  }
}
export function evaluateRule(rule: TaxRule, context: RuleContext, operationDate: string): RuleEvaluation {
  ...
  if (!Number.isFinite(date) || !Number.isFinite(start)) return { ruleId: rule.id, matched: false, applicable: false, missingFields: [], reasons: ['Data de vigência inválida'], confidence: rule.confidence };
  if (date < start || date > end) return { ruleId: rule.id, matched: false, applicable: false, missingFields: [], reasons: ['Regra fora da vigência da operação'], confidence: rule.confidence };
  ...
  if (missingFields.size > 0) return { ruleId: rule.id, matched: false, applicable: false, missingFields: [...missingFields], reasons: ['Dados insuficientes para determinar a aplicabilidade'], confidence: rule.confidence };
  if (reasons.length > 0) return { ruleId: rule.id, matched: false, applicable: false, missingFields: [], reasons, confidence: rule.confidence };
...
export function evaluateRules(rules: TaxRule[], context: RuleContext, operationDate: string): RuleEvaluation[] {
  return [...rules].sort((a, b) => b.priority - a.priority).map(rule => evaluateRule(rule, context, operationDate));
}
